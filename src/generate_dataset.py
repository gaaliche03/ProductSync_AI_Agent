...
cols=["product_name","brands","quantity","categories_en","main_category_en","labels_en",
      "ingredients_text","countries_en","energy-kcal_100g","fat_100g","sugars_100g","proteins_100g","salt_100g"]
...
cat_abbvs = {"beverage":["bev","BEV","Bvrg","boissons"],"snack":["snk","SNACK","Snk.","en-cas"],"dairy":["dair","DAIRY","lait","Ltr"],
             "cereal":["cer","CER","céréal","Cer."],"meat":["meat","MEAT","viande","Mts"],"juice":["jus","JUICE","Jc.","JUS"],
             "bread":["bread","BREAD","pain","Brd"],"sauce":["sce","SAUCE","Sce.","sauces"]}
 
def dirty_category(cat):
    if pd.isna(cat): return np.nan
    cat_lower = str(cat).lower()
    for key, variants in cat_abbvs.items():
        if key in cat_lower:
...
#prix mal formé
df["price"] =df["price"].apply(lambda p: random.choice(prix_formats)(p))
...
df.rename(columns={"product_name":"Prod Name","brands":"Brand","quantity":"Qty/Vol","categories_en":"Cat","main_category_en":"Main Cat",
                   "labels_en":"Labels","ingredients_text":"Desc","countries_en":"Country","energy-kcal_100g":"Kcal","fat_100g":"Fat",
                   "sugars_100g":"Sugar","proteins_100g":"Prot","salt_100g":"Salt","price":"Prix (€)","stock":"Qty Avail",}, inplace=True)
